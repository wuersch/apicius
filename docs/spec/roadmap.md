# Roadmap

> Sequence & priority over time. Status lives on each feature (see `features.md`).
> Indicative, not a commitment — reorder freely.

## Next (candidates, unspecced)
- Capability-first operation creation — resource → capability → derived operation (PRIN-001/002).
- Resource & Shape editing (fields, links, required/auto, ISO formats).
- Example on the noun + inheritance/override (PRIN-005).
- House rules — best practices applied and explained, overridable (PRIN-006).
- Source peek (CodeMirror) + lossless round-trip import/export (PRIN-003/004).

## Later
- Version targeting & downgrade guard (PRIN-007).
- Canvas map (React Flow).
- "Try it" / mock server.
- Relationships: nesting + shared-by-reference.
